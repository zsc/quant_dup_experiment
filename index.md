...
层复制包含两种主要形式：
1. **串行复制**：将单层扩展为多个串行层（Layer_k → Layer_k_copy1 → Layer_k_copy2），利用串行执行的非线性特性产生差异
2. **并行复制**：将单层扩展为多个并行分支，输出进行残差相加（注意：原始输入 x 只加一次，避免重复）

复制层间的差异化来源：
- **权重共享但旋转不同**：复制层共享相同的量化权重，但可在推理时（on-the-fly）应用不同的旋转矩阵
- **激活量化参数差异**：即使权重相同，不同复制层可对输入激活值采用不同的量化超参数（如 scaling factor、clipping threshold）
- **串行非线性累积**：串行执行本身的非线性特性会产生不同的中间表示

### 实验范围

本实验限制在使用校准数据集（calibration set）进行参数优化，不刻意追求完全零训练。选用 2-bit 是因为机器资源限制，同时会探索 1-bit 实验以深入研究方法的极限（即使质量下降较多，只要相比现有方法有所改进即可）。
