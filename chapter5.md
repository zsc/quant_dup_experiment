...
**串行复制场景**：
原始层经过两次旋转变换，先应用旋转矩阵 R1 处理输入，通过 Layer_k 后反旋转得到 Y1，再应用 R2 继续处理，最终反旋转得到输出 Y。
...
**并行复制场景**：
输入 X 分别经过两个并行分支，每个分支使用不同的旋转矩阵（R1 和 R2）处理输入，通过共享权重的 Layer_k，最后将两个分支的输出取平均得到最终结果。

### 5.1.4 旋转参数优化

针对层复制场景，我们需要联合优化多个旋转矩阵。优化目标是最小化总损失函数，该函数包含两部分：量化重建误差（L_quant）和多样性损失（L_diversity），后者鼓励不同旋转产生差异化的量化模式，λ 是平衡两者的系数。
...
1. **量化误差敏感度**：
   计算每层权重量化前后的相对误差，使用 Frobenius 范数衡量差异。

2. **巨大激活值密度**：
   统计每层中超过阈值 τ（通常设为均值的 100 倍）的激活值占比。

3. **梯度敏感度**（使用校准数据近似）：
   结合梯度幅值和量化误差，评估每层对最终损失的影响程度。

### 5.2.2 层重要性评分

综合多个敏感度指标，计算每层的重要性评分，通过加权组合量化敏感度、巨大激活值密度和梯度敏感度三个指标。
...
**网格搜索基线**：
遍历激活缩放因子（s_a）和裁剪阈值（c_a）的不同组合，s_a 范围为 [0.8, 0.9, 1.0, 1.1, 1.2]，c_a 范围为 [0.9, 0.95, 0.99]，评估每种参数组合下的模型困惑度。
...
**多样性损失**：
使用相关矩阵 C 的行列式的负对数作为损失，鼓励不同复制层的输出保持较低的相关性。

**正交性约束**：
通过最小化两个复制层量化误差矩阵（E_1 和 E_2）的内积的 Frobenius 范数，促使它们产生互补的误差模式。
...
借鉴 LoRA 的思想，我们可以在复制层中引入低秩适配，输出由量化权重的主输出和低秩适配项两部分组成。低秩适配使用两个小矩阵 U 和 V（秩通常为 8-16）的乘积，通过缩放系数 α 控制其影响程度，训练时只更新这两个小矩阵。
...
**损失函数设计**：
总损失包含三部分：KL 散度损失用于匹配教师模型输出，多样性损失鼓励复制层之间的差异化，正则化项防止参数偏离过大。三者通过系数 λ_1 和 λ_2 进行平衡。
