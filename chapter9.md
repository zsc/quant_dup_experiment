...
量化流程采用模块化设计：输入模型 → 预处理 → 旋转应用 → 权重量化 → 层复制 → 参数优化 → 评估。每个模块都设计为可插拔组件，便于实验不同的配置组合。
...
1. **串行复制模式**：输入 → Layer_k → Layer_k_copy1 → Layer_k_copy2 → 输出
   - 优势：利用非线性累积产生差异
   - 实现：修改模型 forward 函数，插入复制层

2. **并行复制模式**：输入 → [Layer_k, Layer_k_copy] → 残差相加 → 输出
   - 优势：可并行计算，适合 GPU
   - 注意：原始输入只加一次，避免重复
...
1. **模型权重提取**：PyTorch 模型 → 状态字典 → 张量重排 → 量化 → GGUF 张量
...
1. **复制拓扑信息**：使用 JSON 格式存储每层的复制配置，包括复制类型（串行/并行）、复制数量、权重共享标志和合并策略
